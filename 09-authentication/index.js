...
// Types:
// Stateful auth - auth where server store session data(state) for each logged-in user in memory or db

// Flow:
// user logs in -> server creates a session -> session id is sent back to client ( using cookies ) -> on each further req, client send the session ID -> server looks it up and validate with db -> if valid then allow user to access resource
...
// Flow:
// user logs in -> server generates a JWT token -> token is then send back to client -> client stores it in (localstorage,cookies)
...
// often used to stored session id or JWT
...
// diff between authentiction and authorization?
